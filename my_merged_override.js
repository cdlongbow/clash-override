...
function main(config) {
  ...
  const allProxyNames = proxies.map((p) => p.name).filter(n => n !== '直连' && n !== '拒绝' && !n.includes('中国') && !n.includes('CN') && !n.includes('China') && !n.includes('未知'))
  ...
  functionalGroups.push({
    ...
    name: '默认节点',
    type: 'select',
    proxies: ['自动选择', ...(fallbackGroupNames.length > 0 ? ['故障转移'] : []), ...regionGroupNames, '其他节点', '直连'].filter((n) => n !== '其他节点' || otherProxies.length > 0),
